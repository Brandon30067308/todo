import { MouseEvent, useRef, useState, FC } from 'react';
import './App.css';
import { Input, ListGroupItem, Container, Button, InputGroup, ListGroup } from './styles';
        ...
        <InputGroup onSubmit={addItem}>
          ...
          <div>
            <Button type="submit" className="flex">
              add item <img style={{ marginLeft: '10px' }} className="add-img" src={addImg} alt="add" />
            </Button>
          </div>
        </InputGroup>
        <ListGroup>
          <TodosFilter filter={filter} setFilter={setFilter} />
          {
            ...
            state.todos.map(todo => {
              const isCompleted = state.completed.map(t => t.id).includes(todo.id);
                ...
                <div className="sub-container flex">
                  ...
                  {todo.editState ?
                    ...
                    <span
                      className="text"
                      style={{ opacity: '.735' }}
                      onClick={() => toggleEditState(todo.id)}>{todo.text}
                    </span>
                  }
                </div>
                {/* delete todo button */}
